...
class CIGamEvaler():
    ...
    def forward(self, start_img, goal_img, length=40):
        self.G.eval()
        self.D.eval()
        self.Q.eval()
        self.T.eval()
        plan = self.trainer.plan_hack(0, start_img, goal_img, 0, 'L2', length, save=True)
        return plan.cpu()




class CIGanSimulator():
    def __init__(self):
        ...
        kwargs = vars(args)
        kwargs['loadpath'] = os.path.join('out', 'color_planning_8')
        kwargs['loadepoch'] = 0
        args.prefix = 'sim'
        args.plan_length = 40
        kwargs['color'] = True
        args.learn_var = True
        kwargs['learn_var'] = True
        kwargs['seed'] = 1
        ...
        self.trainer = Trainer(*var_list, **kwargs)
        self.trainer.G.eval()
        self.trainer.D.eval()
        self.trainer.Q.eval()
        self.trainer.T.eval()

    def rollout(self, start_img, goal_img, length=40):
        out = self.trainer.plan_hack(0, torch.from_numpy(start_img[None].astype(np.float32)), torch.from_numpy(goal_img[None].astype(np.float32)), 0, 'L2', length, save=True).cpu()
        outputs = AttrDict(pred_frames=out)
        return outputs
...
if __name__ == "__main__":
    ...
    import os
    s = CIGanSimulator()
    data = np.load(os.path.join('out', 'color_planning_8', 'data-0.npy'))
    start_img = data[0]
    goal_img = data[39]
    length = 40
    print(s.rollout(start_img, goal_img, length))
